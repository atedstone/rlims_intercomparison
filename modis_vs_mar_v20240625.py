...
#
# A copy of the "original" analysis undertaken in modis_vs_mar.py, which was at 6 km on the old MAR grid (and therefore needed the `marutils` package).
#
# This analysis uses 10 km MAR outputs on a Polar Stereog EPSG3413 grid.
#
# Data source: Xavier Fettweis via Horst Machguth, direct download link, June 2024. Metadata in netcdf files indicate MAR v3.14.
...
# %% trusted=true
pth_mar = '/flash/tedstona/MARv.HorstRCMStudy_20240624/'
...
mar_eg = mar_eg.rio.write_crs('epsg:3413')
mar_eg = reorganise_mar(mar_eg)

# %% trusted=true
# IF NEW DEM NEEDED: SEE NEXT SECTION OF NOTEBOOK !
dem = rioxarray.open_rasterio(os.path.join(pth_project, 'arcticdem_mosaic_500m_v30_greenland_icesheet_GeoidCorr_GapFilled_MAR10km.tif')).squeeze()
...
# %% trusted=true
# gdalwarp -tr 10000 10000 -te -645000 -3352927.75 855000 -652927.75 arcticdem_mosaic_500m_v30_greenland_icesheet_GeoidCorr_GapFilled.tif arcticdem_mosaic_500m_v30_greenland_icesheet_GeoidCorr_GapFilled_MAR10km.tif
...
# %% trusted=true
runoff = xr.open_mfdataset(os.path.join(pth_mar, '*.nc'), preprocess=lambda x:x.RU)
runoff = runoff.rio.write_crs('epsg:3413')
runoff = reorganise_mar(runoff)
...
# %% trusted=true
plt.figure()
(ru_annual.where(aois.sel(aoi=156)).sum(dim=('x','y')) / 10 / 100 * (MAR_RES**2) / 1e9).plot()
...
ru_annual.where(aois.sel(aoi=156)).squeeze().plot(col='TIME', col_wrap=5, cmap='viridis', vmin=0, vmax=100)
...
# %% trusted=true
aoi_ru_save.to_netcdf(os.path.join(pth_project, 'MAR-v.20240624-rlim-RUa1mm.nc'))
...
# %% trusted=true
mar_rlim = xr.open_dataset(os.path.join(pth_project, 'MAR-v.20240624-rlim-RUa1mm.nc'))
...
# %% trusted=true
rlim_pd.to_csv(os.path.join(pth_project, 'MAR-v.20240624-rlim-RUa1mm.csv'))
...
for year in range(2000, 2024):
    ...
    rlims_here[rlims_here > 0].to_csv(os.path.join(pth_project,'flowlines_daily_rlims_MAR-v.20240624_%smmEvents_%smmAnnual_%s.csv' %(t_daily, t_annual, year)))
...
for year in range(2000, 2022):
    p = os.path.join(pth_project,'flowlines_daily_rlims_MAR-v.20240624_1mmEvents_10mmAnnual_%s.csv' %year)
    d = pd.read_csv(p)
    s.append(d)
...
# %% trusted=true
all_mar.to_csv(os.path.join(pth_project, 'flowlines_daily_rlims_MAR-v.20240624_1mmEvents_10mmAnnual_2000_2021.csv'))
